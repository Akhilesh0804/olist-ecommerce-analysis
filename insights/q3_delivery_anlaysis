Problem: Does delivery time affect customer satisfaction scores, and if so, at what point does a longer delivery duration cause ratings to drop significantly?

Approach: Before bucketing, a percentile distribution query was run to determine data driven thresholds. The analysis revealed P25 at 6 days, median at 10 days, P75 at 15 days, and P90 at 23 days. Buckets were set at these natural breakpoints rather than arbitrary thresholds, ensuring each bucket represents a meaningful segment of the customer base. A CTE was used to first calculate delivery duration per order and assign each order to a bucket, followed by an aggregation query to calculate the average review score and order count per bucket.

Output:
| delivery_bucket | order_count | avg_review_score | avg_delivery_days |
|---|---|---|---|
